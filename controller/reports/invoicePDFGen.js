...
async function generateInvoicePDF(invoiceId) {
  try {
    ...
    invoice.items.forEach(item => {
      doc.text(item.description, 30, doc.y, { width: 150, continued: true })
        .text(item.quantity.toString(), 200, doc.y, { width: 50, continued: true })
        .text(`KSH${item.amount.toFixed(2)}`, 260, doc.y, { width: 100, continued: true })
        .moveDown();
    });
