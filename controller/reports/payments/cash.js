...
async function getCashPaymentsReport(req, res) {
  let doc;
  try {
    ...
    if (!payments.length) {
      return res.status(404).json({ message: "No cash payments found." });
    }

    doc = new PDFDocument({ size: 'A4', margin: 40 });
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', 'attachment; filename="cash_payments_report.pdf"');
    doc.pipe(res);
    ...
    doc.fontSize(14)
       .font('Helvetica-Bold')
       .text('Cash Payments Report', { align: 'center' })
       .moveDown(1);

    const startX = 40;
    let currentY = doc.y + 20;
    const columnWidths = [120, 80, 80, 80];
    const headers = ['Customer Name', 'Receipt No', 'Amount', 'Date'];
    ...
    for (const payment of payments) {
      ...
      doc.text(customerName.substring(0, 20), startX, currentY)
         ...
         .text(date, startX + columnWidths[0] + columnWidths[1] + columnWidths[2], currentY);
      currentY += 15;
    }
    ...
    doc.font('Helvetica-Bold')
       .text('TOTAL CASH AMOUNT:', startX, currentY)
...
module.exports = { getCashPaymentsReport };