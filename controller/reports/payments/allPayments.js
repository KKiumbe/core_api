...
async function getAllPaymentsReport(req, res) {
  let doc;
  try {
    ...
    res.setHeader('Content-Disposition', 'attachment; filename="all_payments_report.pdf"');
    doc.pipe(res);
    ...
    doc.fontSize(14)
       .font('Helvetica-Bold')
       .text('All Payments Report', { align: 'center' })
       .moveDown(1);

    const startX = 40;
    let currentY = doc.y + 20;
    const columnWidths = [120, 80, 80, 80, 80];
    const headers = ['Customer Name', 'Receipt No', 'Amount', 'Mode', 'Date'];
    ...
    for (const payment of payments) {
      ...
      doc.text(customerName.substring(0, 20), startX, currentY)
         ...
         .text(payment.modeOfPayment, startX + columnWidths[0] + columnWidths[1] + columnWidths[2], currentY)
    ...
    console.error('Error generating all payments report:', error);
    if (!res.headersSent) {
...
module.exports = { getAllPaymentsReport };