...
async function getCustomersWithLowBalance(req, res) {
  ...
  try {
    ...
    if (!filteredCustomers.length) {
      return res.status(404).json({ message: "No customers with low balances found." });
    }

    // Create PDF document
    doc = new PDFDocument({ size: 'A4', margin: 40 });
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', 'attachment; filename="low_balance_customers_report.pdf"');
    doc.pipe(res);
    ...
    doc.fontSize(14)
       .font('Helvetica-Bold')
       .text('Low Balance Customers Report', { align: 'center' })
       .moveDown(1);
    ...
    // Total balances
    const totalBalance = filteredCustomers.reduce((sum, c) => sum + (c.closingBalance || 0), 0);
    currentY += 10;
    if (currentY > 700) {
      doc.addPage();
      await generatePDFHeader(doc);
    ...
    doc.font('Helvetica-Bold')
       .text('TOTAL LOW BALANCE:', startX, currentY)
    ...
    console.error('Error generating low balance customers report:', error);
    if (!res.headersSent) {
      res.status(500).json({ 
...
async function getCustomersWithHighDebt(req, res) {
  ...
  try {
    ...
    doc = new PDFDocument({ size: 'A4', margin: 40 });
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', 'attachment; filename="high_debt_customers_report.pdf"');
    doc.pipe(res);
    ...
    doc.fontSize(14)
       .font('Helvetica-Bold')
       .text('High Debt Customers Report', { align: 'center' })
       .moveDown(1);
    ...
    // Total balances
    const totalDebt = filteredCustomers.reduce((sum, c) => sum + (c.closingBalance || 0), 0);
    currentY += 10;
    if (currentY > 700) {
      doc.addPage();
      await generatePDFHeader(doc);
    ...
    currentY += 10;
    doc.font('Helvetica-Bold')
       .text('TOTAL HIGH DEBT:', startX, currentY)
       .text(totalDebt.toFixed(2), startX + columnWidths[0] + columnWidths[1] + columnWidths[2] + columnWidths[3], currentY);
    ...
    console.error('Error generating high debt customers report:', error);
    if (!res.headersSent) {
      res.status(500).json({ 
        message: 'Error generating report', 
        error: error.message 
